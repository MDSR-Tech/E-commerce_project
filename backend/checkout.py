...
def send_order_confirmation_email(user, order):
    """Send order confirmation email (skipped in production due to SMTP restrictions)"""
    # Skip email in production (Railway blocks SMTP)
    if os.getenv('RAILWAY_ENVIRONMENT') or os.getenv('SKIP_EMAILS'):
        print(f"[PROD] Skipping order confirmation email for order #{order.id} to {user.email}")
        return
