...
def send_order_cancellation_email(user, order):
    """Send order cancellation email (skipped in production due to SMTP restrictions)"""
    # Skip email in production (Railway blocks SMTP)
    if os.getenv('RAILWAY_ENVIRONMENT') or os.getenv('SKIP_EMAILS'):
        print(f"[PROD] Skipping order cancellation email for order #{order.id} to {user.email}")
        return
    ...
    try:
        ...
        msg = Message(
            subject=f'Order Cancelled - MDSRTech #{order.id}',
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <div style="background-color: #fef2f2; border-radius: 8px; padding: 20px; margin: 20px 0; border: 1px solid #fecaca;">
                    ...
                    <p style="margin: 5px 0 0; color: #dc2626; font-size: 20px; font-weight: bold;">#{order.id}</p>
                </div>
                ...
                <div style="border-top: 2px solid #e5e7eb; padding-top: 15px; margin-top: 15px;">
                    <div style="display: flex; justify-content: space-between; font-size: 18px; font-weight: bold;">
                        ...
                        <span style="color: #dc2626;">${order.total_cents / 100:.2f} CAD</span>
                    </div>
                </div>
        ...
        mail.send(msg)
        print(f"Order cancellation email sent for order #{order.id}")
